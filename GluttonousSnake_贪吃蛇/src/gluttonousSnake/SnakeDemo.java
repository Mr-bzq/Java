...
public class SnakeDemo extends JComponent{
    ...
    public void Thread(){
        long millis = 200;//每隔300毫秒刷新一次，蛇速度
        run = new Thread() {
            public void run() {
                while (true) 
                {
    ...
    public static void main(String[] args) {
        ...
        game.setTitle("贪吃蛇");//标题
        game.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        //设置用户在此窗体上发起 "close" 时默认执行的操作。
//        EXIT_ON_CLOSE 退出应用程序后的默认窗口关闭操作。如果
//        执行关闭操作时窗口具有此设置，并且是在 applet 中关闭窗口，
//        则可能抛出 SecurityException。建议仅在应用程序中使用此设置
//       game.setSize(502, 507);
        game.setSize(610, 507);//调整组件的大小，使其宽度为 width，高度为 height。
        game.setResizable(false);//设置此窗体是否可由用户调整大小。
        game.setLocationRelativeTo(null);//设置窗口相对于指定组件的位置
       
        game.add(t);//将指定组件追加到此容器的尾部
        game.setVisible(true);//根据参数 b 的值显示或隐藏此 Window。
    }
